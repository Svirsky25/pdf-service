	...
	public async generatePdf(data: PdfRequest): Promise<Buffer> {
		const browser = await puppeteer.launch({
			...
			args: [
				...
				"--disable-dev-shm-usage", // קריטי למניעת קריסות זיכרון
				"--disable-gpu", // חוסך משאבים בשרת
				"--no-zygote", // אופטימיזציה לזיכרון
			],
		});
		...
		try {
			...
			try {
				...
				throw new Error(
					`Content Loading Error: ${contentError.message}. This often happens due to broken external links or images.`,
				);
			}
